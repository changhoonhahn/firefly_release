...
class newFitter(object): 
    ...
    """
    def __init__(self, wavelength, data, error, models, upper_limit_fit=None, fit_cap=None):
        self.wavelength = wavelength 
        self.data = data
        self.error = error
        self.models = models
        self.upper_limit_fit = upper_limit_fit
        ...
        self.chi_models = np.zeros(np.shape(models))
        for m in range(self.num_models):
            self.chi_models[m] = (models[m]-data)/error
        ...
        for im in range(len(models)):
            ...
            chi_arr = np.dot(fit_first['weights'][im], self.chi_models[im])
        ...
        self.clipped_arr = fit_list[np.argmin(int_chi)]['clipped_arr']

        # Fit_list is our initial guesses from which we will iterate
        final_fit_list = self._iterate(fit_list)
    ...
    def output(self):  
        weights = np.array([fdict['weights'] for fdict in self.total_fit_list]) 
        chis = np.array([fdict['chi_squared'] for fdict in self.total_fit_list]) 
        branches = np.array([fdict['branch_num'] for fdict in self.total_fit_list]) 
        return weights, chis, branches

    def _iterate(self, fitlist): 
        ...
        for f in range(len_list):
            new_list = self._spawn_children(fitlist[f], self.iterate_count) 
	    ...
	    for n in range(len_new):
                ...
                if new_chi < (previous_chis - save_bic): # If they do, add them to the fit list!
                    count_new += 1
                    if count_new > self.fit_cap:
                        break
                    fitlist.append(new_list[n])

            if count_new > self.fit_cap: break
        ...
        if count_new == 0:
            ...
            fit_list_new = self._iterate(fitlist)
            return fit_list_new

    def _spawn_children(self, fitdict, branch_num):
        ...
        for im in range(self.num_models): 
            new_weights[im] += 1
